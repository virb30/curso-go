...
func (s *OrderService) CreateOrder(ctx context.Context, in *pb.CreateOrderRequest) (*pb.CreateOrderRsesponse, error) {
	...
	return &pb.CreateOrderRsesponse{
		Id:         output.ID,
...
func (s *OrderService) ListOrders(ctx context.Context, in *pb.Blank) (*pb.OrdersList, error) {
	...
	var ordersOutput []*pb.Order
	for _, order := range output.Orders {
		ordersOutput = append(ordersOutput, &pb.Order{
			Id:         order.ID,
